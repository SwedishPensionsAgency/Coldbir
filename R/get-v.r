...
get_v <- function(name, path = getwd(), dims = NULL, na = NA) {
  ...
  if (bytes <= 4) {
    ...
    #if (H_itemSize == 1) data[data == -128] 	<- as.integer(NA)
    #if (H_itemSize == 2) data[data == -32768] 	<- as.integer(NA)
  } else {	
  ...
  if (type == "i") {
    ...
    if(exponent > 0)	x <- x / 10.0^exponent
    if(!is.na(na)) x[is.na(x)] <- as.double(na)
  } else if(type == "l") {
    x <- (x > 0L)
